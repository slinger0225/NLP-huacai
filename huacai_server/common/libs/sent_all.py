...
def sent_rec(content):#传入用户的id，用户文本内容
	...
	for items in reco:
		...
		com_list.update(pic_num=songsheet.cell(items, 14).value)
		com_list.update(song_name=songsheet.cell(items, 16).value)
		com_list.update(singer=songsheet.cell(items, 15).value)
		com_list.update(com1=songsheet.cell(items, 9).value)
		com_list.update(com2=songsheet.cell(items, 10).value)
		com_list.update(com3=songsheet.cell(items, 11).value)
	...
	reco_dict = {str("music_info"): reco_all, 'sentiment': sent[list(max_sentiment)[0]]}
	return reco_dict
